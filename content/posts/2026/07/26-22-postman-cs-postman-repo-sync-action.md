---
title: Postman Onboarding Repo Sync
date: 2026-07-26 22:35:43 +00:00
tags:
  - postman-cs
  - GitHub Actions
draft: false
repo: https://github.com/postman-cs/postman-repo-sync-action
marketplace: https://github.com/marketplace/actions/postman-onboarding-repo-sync
version: v2.1.16
dependentsNumber: "0"
actionType: Node
nodeVersion: 24
actionSummary: |
  This GitHub Action automates the process of exporting Postman collections, environments, and other resources into a repository. It also sets up CI for testing, integrates mock servers, and monitors API usage. The action is part of the Postman API Onboarding suite and requires permissions to write to the repository and manage Actions secrets.
---


Version updated for **https://github.com/postman-cs/postman-repo-sync-action** to version **v2.1.16**.

- This action is used across all versions by **0** repositories.

## Action Type
This is a **Node** action using Node version **24**.

Go to the [GitHub Marketplace](https://github.com/marketplace/actions/postman-onboarding-repo-sync) to find the latest changes.

## Action Summary

This GitHub Action automates the process of exporting Postman collections, environments, and other resources into a repository. It also sets up CI for testing, integrates mock servers, and monitors API usage. The action is part of the Postman API Onboarding suite and requires permissions to write to the repository and manage Actions secrets.

## What's Changed

## What's Changed
* ci(release): port automatic release cut by @jaredboynton in https://github.com/postman-cs/postman-repo-sync-action/pull/102
* fix(release): recover unpublished immutable tags by @jaredboynton in https://github.com/postman-cs/postman-repo-sync-action/pull/103


**Full Changelog**: https://github.com/postman-cs/postman-repo-sync-action/compare/v2...v2.1.16
