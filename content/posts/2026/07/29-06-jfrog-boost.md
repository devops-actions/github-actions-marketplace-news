---
title: JFrog Boost
date: 2026-07-29 06:55:06 +00:00
tags:
  - jfrog
  - GitHub Actions
draft: false
repo: https://github.com/jfrog/boost
marketplace: https://github.com/marketplace/actions/jfrog-boost
version: v0.9.22
dependentsNumber: "4"
actionType: Composite
actionSummary: |
  Boost is a tool designed to save tokens while maintaining context for coding agents. It trims unnecessary log details, such as timestamps and changed counts, to keep only the essential information needed for analysis and debugging. This helps in reducing noise, allowing agents to focus on their tasks without overwhelming with irrelevant data.
---


Version updated for **https://github.com/jfrog/boost** to version **v0.9.22**.
- This publisher is shown as 'verified' by GitHub.

- This action is used across all versions by **4** repositories.

## Action Type
This is a **Composite** action.

Go to the [GitHub Marketplace](https://github.com/marketplace/actions/jfrog-boost) to find the latest changes.

## Action Summary

Boost is a tool designed to save tokens while maintaining context for coding agents. It trims unnecessary log details, such as timestamps and changed counts, to keep only the essential information needed for analysis and debugging. This helps in reducing noise, allowing agents to focus on their tasks without overwhelming with irrelevant data.

## What's Changed

## What's Changed
* Release v0.7.23 by @yahav-ohana in https://github.com/jfrog/boost/pull/41
* Release v0.7.25 by @menachemm-byte in https://github.com/jfrog/boost/pull/44
* docs(readme): simplify mascot, focus on token savings, add report commands by @yahav-ohana in https://github.com/jfrog/boost/pull/47
* docs(readme): update release badge to v0.8.6 and stars to 258 by @yahav-ohana in https://github.com/jfrog/boost/pull/48
* docs(readme): add how-to-use walkthrough GIF above Quick start by @yahav-ohana in https://github.com/jfrog/boost/pull/52
* docs(readme): add Boost comparison table by @yahav-ohana in https://github.com/jfrog/boost/pull/53
* fix: install.ps1 by @menachemm-byte in https://github.com/jfrog/boost/pull/54
* docs: refresh README badges, install, use cases, and agent guide by @yahav-ohana in https://github.com/jfrog/boost/pull/56

## New Contributors
* @menachemm-byte made their first contribution in https://github.com/jfrog/boost/pull/44

**Full Changelog**: https://github.com/jfrog/boost/compare/v0.7.23...v0.9.22
