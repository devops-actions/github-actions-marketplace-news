---
title: SSG - Static Site Generator
date: 2026-07-14 15:02:04 +00:00
tags:
  - spagu
  - GitHub Actions
draft: false
repo: https://github.com/spagu/ssg
marketplace: https://github.com/marketplace/actions/ssg-static-site-generator
version: v1.8.3
dependentsNumber: "0"
actionType: Composite
actionSummary: |
  SSG is a fast static site generator written in Go that converts Markdown with YAML frontmatter into a complete website, handling clean URLs, templates, feeds, search, and more. It supports various deployment options including native support for Cloudflare Pages, GitHub Pages, Netlify, Vercel, FTP, and SFTP.
---


Version updated for **https://github.com/spagu/ssg** to version **v1.8.3**.

- This action is used across all versions by **0** repositories.

## Action Type
This is a **Composite** action.

Go to the [GitHub Marketplace](https://github.com/marketplace/actions/ssg-static-site-generator) to find the latest changes.

## Action Summary

SSG is a fast static site generator written in Go that converts Markdown with YAML frontmatter into a complete website, handling clean URLs, templates, feeds, search, and more. It supports various deployment options including native support for Cloudflare Pages, GitHub Pages, Netlify, Vercel, FTP, and SFTP.

## What's Changed

## Installation

### Quick Install (Linux/macOS)
```bash
curl -sSL https://raw.githubusercontent.com/spagu/ssg/main/install.sh | bash
```

### Package Managers
- **Homebrew**: `brew install spagu/tap/ssg`
- **Snap**: `snap install ssg`
- **Debian/Ubuntu**: Download `.deb` file below
- **Fedora/RHEL**: Download `.rpm` file below

### Checksums
See `checksums.sha256` for file verification.

📖 Full documentation: https://github.com/spagu/ssg#readme


## What's Changed
* feat(1.8.3): template query helpers, image processing, SCSS, skip-links, perf batch by @spagu in https://github.com/spagu/ssg/pull/23


**Full Changelog**: https://github.com/spagu/ssg/compare/v1.8.2...v1.8.3
