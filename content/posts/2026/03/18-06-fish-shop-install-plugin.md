---
title: fish-shop/install-plugin
date: 2026-03-18 06:05:21 +00:00
tags:
  - fish-shop
  - GitHub Actions
draft: false
repo: https://github.com/fish-shop/install-plugin
marketplace: https://github.com/marketplace/actions/fish-shop-install-plugin
version: v2.3.98
dependentsNumber: "31"
actionType: Composite
---


Version updated for **https://github.com/fish-shop/install-plugin** to version **v2.3.98**.
- This action is used across all versions by **31** repositories.

## Action Type
This is a **Composite** action.

Go to the [GitHub Marketplace](https://github.com/marketplace/actions/fish-shop-install-plugin) to find the latest changes.

## Action Summary

The `fish-shop/install-plugin` GitHub Action automates the installation of plugins for the Fish shell, leveraging supported plugin managers like Fisher and Oh My Fish. It simplifies workflows by ensuring the plugin manager is installed (if not already present) and securely handles plugin installation, supporting multiple plugins in a single step. This action addresses the need for streamlined and secure plugin management in CI/CD pipelines using Fish shell environments.

## Release notes

## What's Changed

* Bump the version-updates group with 8 updates by @dependabot[bot] in https://github.com/fish-shop/install-plugin/pull/382

**Full Changelog**: https://github.com/fish-shop/install-plugin/compare/v2.3.97...v2.3.98
